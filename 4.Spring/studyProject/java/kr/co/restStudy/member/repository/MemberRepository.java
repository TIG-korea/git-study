...
public interface MemberRepository extends JpaRepository <Member, Long>{
	 ...
	 * - SELECT count(*) > 0 FROM member WHERE user_id = ?   (existsByUserId)
	 * 
	 * 
	 ...
	 * 이메일 중복 체크
	 * - JPA 메서드 이름 규칙에 따라 자동으로 쿼리가 생성됨
	 * - SELECT count(*) > 0 FROM member WHERE email = ?   (existsByEmail)
	 * 
	 * 
	 ...
	 * - JPA 메서드 이름 규칙에 따라 자동으로 쿼리가 생성됨
	 * 		> findBy 필드명(매개변수)
	 * 		> SELECT * FROM member WHERE user _ id = ?
	 * 
	 * 주의할 점
	 * 	- 현재 반환 타입은 엔티티(Member)를 받으므로 1개만 반환 가능
	 * 		> 동일한 usdrId는 존재할 수 없다고 가정 (UNIQUE 제약)
	 * 
	 * 		참고
	 * 	- 여러개의 SELECT 결과를 받을 경우 List가 사용됨 
